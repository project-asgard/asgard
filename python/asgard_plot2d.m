function [z, x, y] = asgard_plot2d(filename, point_list, num_points)
...
if (size(point_list, 1) ~= 1)
    error('the input list must be 1d, e.g., {0.1, [], [1, 2]}')
end
...
for i = 1:nentry
    if (sum(size(point_list{i})) ~= 0)
        if (size(point_list{i}, 1) ~= 1 || (size(point_list{i}, 2) ~= 1 && size(point_list{i}, 2) ~= 2))
            error(['incorect shape for entry ', num2str(i), ' point_list should look like {[1, 1.5], [1, 2]} or {[], [], -0.3}']);
        end
...
if (num_w2d ~= 2)
    error('must specify two dimenisons with a range, e.g., {0.1, [-1, 1], []}');
end
...
command = [asgardpy_exe, ' -plot2d ', filename];
