...
double test_moments(std::vector<P> const &drange, int level, int degree,
                    ...
                    std::vector<std::function<P(P)>> const moments)
{
  ...
  std::vector<discretization_manager<P>> dmoms;

  for (int m = 0; m < num_moms; m++)
  {
    dmoms.emplace_back(
      discretization_manager<P>(
  ...
  int const num_mom = static_cast<int>(moments.size());
  ...
  std::vector<std::vector<P>> vmoms(num_mom, std::vector<P>(raw_moments.size() / num_mom));
  {
    std::vector<decltype(vmoms.front().begin())> imoms(num_mom);
    for (int m : iindexof(num_mom))
      imoms[m] = vmoms[m].begin();

    auto im = raw_moments.begin();
    while (imoms.front() != vmoms[0].end()) {
      for (int i : iindexof(num_mom))
      {
...
TEST_CASE("compute moments", "[moments]")
{
  ...
  SECTION("3D")
  {
    std::vector<std::function<P(P)>> base(3), moms(3);
    ...
    moms[1] = [](P x) -> P { return 8.124814981273536 * -2.067472642818473e-02 * std::sin(x); };
    moms[2] = [](P x) -> P { return 12.93871499200409 * 0.393141134391177 * std::sin(x); };
  ...
  SECTION("4D")
  {
    std::vector<std::function<P(P)>> base(4), moms(3);
    ...
    moms[1] = [](P x) -> P {
      return 6.162820236651310e-02 * 8.124814981273536 * -2.067472642818473e-02 * std::sin(x);
    };
    moms[2] = [](P x) -> P {
      return -8.908119100126307e-03 * 12.93871499200409 * 0.393141134391177 * std::sin(x);
    };
