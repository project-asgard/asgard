...
#pragma omp parallel for
  ...
  auto weight = [=](P /* time */, asgard::vector2d<P> const& /* nodes */,
                 ...
                 std::vector<P> &vals)
    {
      std::vector<P> const &ef = moments[melectric];
#pragma omp parallel for
      for (size_t i = 0; i < vals.size(); i++)
        vals[i] = field[i] * ef[i] * ef[i];
    };

  pde.set_adapt_weight(weight, {melectric});
...
void test_energy(std::string const &opt_str) {
  ...
  for (int64_t i = 0; i < n; i++)
  {
    ...
    auto efield = disc.get_moment_level(melectric_x);
    ...
    // std::cout << std::abs(0.5 * (Ep + Ek) - E0) << std::endl;
