...
TEMPLATE_TEST_CASE("tpsv", "[lib_dispatch]", test_precs)
{
  ...
  fk::vector<TestType> const X_gold{0.22222222222222220989, 0.05555555555555557329, 0.16666666666666665741};
  ...
  {
    fk::vector<TestType, mem_type::owner, resource::device> A_gold = A_packed_gold.clone_onto_device();
    fk::vector<TestType, mem_type::owner, resource::device> x = B_gold.clone_onto_device();
    int ldb = x.size();

    lib_dispatch::tpsv<resource::device>('U', 'N', 'N', ldb, A_gold.data(), x.data(), 1);
    auto constexpr tol_factor = get_tolerance<TestType>(10);
