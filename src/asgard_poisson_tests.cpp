                                 ...
                                 int degree, int level)
{
  ...
  auto build_func = [&terms](term_entry<P> &tentry, int const dim, int const level) -> void {
    terms.rebuild_term1d(tentry, dim, level);
  };
  #ifdef ASGARD_USE_GPU
  auto iter_solve_func = [&poisson_iter](solvers::operation_apply_lhs<P> apply_lhs,
                                gpu::vector<P> const &rhs, gpu::vector<P> &x) -> int {
    return poisson_iter.solve(apply_lhs, rhs, x);
  };
  #else
  auto iter_solve_func = [&poisson_iter](solvers::operation_apply_lhs<P> apply_lhs,
                                std::vector<P> const &rhs, std::vector<P> &x) -> int {
    return poisson_iter.solve(apply_lhs, rhs, x);
  };
  #endif
