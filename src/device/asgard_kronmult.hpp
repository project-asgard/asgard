...
void gpu3d(T const * const Aarray_[],
  ...
  static_assert( n==2 or n==3, "unimplemented size n (i.e., polynomial degree)");

  constexpr int max_blocks = 300;
  if constexpr (n == 2){
    int constexpr num_threads = 1024;
    int num_blocks = std::min( max_blocks, (batchCount + num_threads / 8 - 1) / (num_threads / 8) ); // one operation takes two threads
    kernel::gpu3d<T, num_threads, 2><<<num_blocks, num_threads>>>(Aarray_, lda, pX_, pY_, batchCount);
  } else if constexpr (n == 3){
    int constexpr num_threads = 32;
    int num_blocks = std::min( max_blocks, (batchCount + num_threads / 27 - 1) / (num_threads / 27) ); // one operation takes two threads
    kernel::gpu3d<T, num_threads, 3><<<num_blocks, num_threads>>>(Aarray_, lda, pX_, pY_, batchCount);
  }
}
...
/*
 * The CPU kernels can be better. The main issue is the atomic operations and matrix lda,
 * but we could speed this up with some intrinsics too.
 */
template<typename T, int n>
void cpu1d(T const * const Aarray_[],
           int const lda,
           T* pX_[],
           T* pY_[],
           int const num_batch)
{
  ...
  for(int i=0; i<num_batch; i++){
    if constexpr (n == 2){
      omp_atomic_add(&pY_[i][0], Aarray_[i][0] * pX_[i][0] + Aarray_[i][lda] * pX_[i][1]);
      omp_atomic_add(&pY_[i][1], Aarray_[i][1] * pX_[i][0] + Aarray_[i][lda+1] * pX_[i][1]);
    }else if constexpr (n==3){
      omp_atomic_add(&pY_[i][0], Aarray_[i][0] * pX_[i][0] + Aarray_[i][lda] * pX_[i][1] + Aarray_[i][2*lda] * pX_[i][2]);
      omp_atomic_add(&pY_[i][1], Aarray_[i][1] * pX_[i][0] + Aarray_[i][lda+1] * pX_[i][1] + Aarray_[i][2*lda+1] * pX_[i][2]);
      omp_atomic_add(&pY_[i][2], Aarray_[i][2] * pX_[i][0] + Aarray_[i][lda+2] * pX_[i][1] + Aarray_[i][2*lda+2] * pX_[i][2]);
    }else if constexpr (n==4){
      omp_atomic_add(&pY_[i][0], Aarray_[i][0] * pX_[i][0] + Aarray_[i][lda] * pX_[i][1] + Aarray_[i][2*lda] * pX_[i][2] + Aarray_[i][3*lda] * pX_[i][3]);
      omp_atomic_add(&pY_[i][1], Aarray_[i][1] * pX_[i][0] + Aarray_[i][lda+1] * pX_[i][1] + Aarray_[i][2*lda+1] * pX_[i][2] + Aarray_[i][3*lda+1] * pX_[i][3]);
      omp_atomic_add(&pY_[i][2], Aarray_[i][2] * pX_[i][0] + Aarray_[i][lda+2] * pX_[i][1] + Aarray_[i][2*lda+2] * pX_[i][2] + Aarray_[i][3*lda+2] * pX_[i][3]);
      omp_atomic_add(&pY_[i][3], Aarray_[i][3] * pX_[i][0] + Aarray_[i][lda+3] * pX_[i][1] + Aarray_[i][2*lda+3] * pX_[i][2] + Aarray_[i][3*lda+3] * pX_[i][3]);
    }else{
...
template<typename T, int n>
void cpu2d(T const * const Aarray_[],
           int const lda,
           T* pX_[],
           T* pY_[],
           int const num_batch){
