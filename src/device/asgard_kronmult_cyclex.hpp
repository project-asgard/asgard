                       ...
                       T *pY[], int const num_batch)
{
  ...
  static_assert(num_cycles >= 1, "invalid number of cycles, must be at least 1");
  static_assert(num_cycles*team_size >= int_pow<n, dims>(), "team is too small, team_size X num_cycles must be at least n^dims");
  ...
  constexpr int num_last_cycle = int_pow<n, dims>() - (num_cycles - 1) * team_size;
  static_assert(num_last_cycle > 0, "too many cycles, the last one is pointless");
  ...
  while (i < num_batch)
  {
    for(int c=0; c<num_cycles-1; c++){
        X[threadIdx.y][threadIdx.x + c*team_size] = pX[i][threadIdx.x + c*team_size];
    }
