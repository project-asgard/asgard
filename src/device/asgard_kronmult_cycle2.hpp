...
__global__ void
cycle2(int const num_batch, int const num_cols, int const num_terms,
       int const iA[], P const * const vA[], P const alpha, P const x[], P y[])
{
  static_assert(dims <= 6, "kernel won't work for more than 6 dimensions");
  static_assert(
      2 * team_size >= int_pow<n, dims>(),
  ...
  //__shared__ P const* pA[1024];
  //if (threadIdx.x + blockDim.x * threadIdx.y < num_terms)
  //  pA[threadIdx.x + blockDim.x * threadIdx.y] = vA[threadIdx.x + blockDim.x * threadIdx.y];
  //__syncthreads();
  ...
  while (i < num_batch)
  {
    ...
    //if (threadIdx.x < dims)
    //  ia[threadIdx.y][(6 - dims) + threadIdx.x] = iA[i * dims + threadIdx.x];
    //__syncthreads();
    ...
    for (int t = 0; t < num_terms; t++)
    {
      ...
      {
        if (threadIdx.x < n * n)
          A[threadIdx.y][threadIdx.x] = vA[t][iA[ma++] + threadIdx.x];
