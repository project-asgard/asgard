 ...
 *         making it a template parameter allows some operations to happen at compile time.
 ...
 *
 * \param Aarray is pointer to all matrices for the products in the batch
 ...
 * - j is index of this thread within the group of threads that work on the batch
...
__global__ void gpu1d(T const * const Aarray[],
  ...
  while(i < num_batch){
    ...
    X[threadIdx.x] = pX[i][j]; // read the X, every 2 threads read consecutive entries and store in cache
