...
TEMPLATE_TEST_CASE("testing contuinity 2 implementations, level 5, degree 4", "[pde]", double,
                   float)
{
  int const level = 5;
  int const degree = 4;
  ...
  SECTION("continuity 2 initial condition functions")
  {
    for (int i = 0; i < pde->num_dims; ++i)
    {
      fk::vector< TestType > const gold(read_vector_from_txt_file(
          base_dir + "initial_dim" + std::to_string(i) + ".dat"));
      fk::vector< TestType > const fx = pde->get_dimensions()[i].initial_condition(x, t);
  ...
  SECTION("continuity 2 exact solution functions")
  {
    ...
    fk::vector< TestType > const gold( read_vector_from_txt_file(base_dir + "exact_time.dat"));
    fk::vector< TestType > fx( gold.size() );
    std::transform( x.begin(), x.end(), fx.begin(), [&pde]( TestType const x ) -> TestType
                    { return pde->exact_time(x); });
    rmse_comparison( fx, gold, tol_factor );
  }
  ...
  SECTION("continuity 2 source functions")
  {
    for (int i = 0; i < pde->num_sources; ++i)
    {
      ...
      fk::vector< TestType > const gold( read_vector_from_txt_file(source_string + "time.dat") );

      fk::vector< TestType > fx( gold.size() );
      std::transform( x.begin(), x.end(), fx.begin(), [ &pde, i ]( TestType const x ) -> TestType
