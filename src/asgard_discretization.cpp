...
discretization_manager<precision>::discretization_manager(
    ...
    : verb(verbosity), pde(std::move(pde_in)), grid(*pde), matrices(pde.get()),
      transformer(*pde, verb), degree_(0), dt_(0), time_(0), time_step_(0),
      final_time_step_(0), kronops(verbosity)
{
  ...
  while (keep_working)
  {
    ...
    for (auto i : indexof(num_initial))
    {
      ...
      hier.template project_separable<data_mode::increment>
          (state.data(), dims, fx, matrices.dv(), matrices.mass(), grid, 0,
...

  {
    ...
    for (auto const &source : pde->sources())
      hier.template project_separable<data_mode::increment>
          ...
          matrices.dv(), matrices.mass(), grid, t, source.time_func()(t));
  }{
    ...
    precision t, std::vector<precision> const &x, std::vector<precision> &R) const
{
  ...
  if (not pde->sources().empty())
  {
    ...
    hier.template project_separable<data_mode::replace>
          ...
          matrices.dv(), matrices.mass(), grid, t, sources[0].time_func()(t));

    for (size_t i = 1; i < sources.size(); i++)
      hier.template project_separable<data_mode::increment>
          (R.data(), pde->get_dimensions(), sources[i].source_funcs(),
          matrices.dv(), matrices.mass(), grid, t, sources[i].time_func()(t));
  }
                                               ...
                                               std::vector<precision> &x) const
{
  ...
  switch (solver)
  {
  case solve_opts::scalapack:
    throw std::runtime_error("cannot use discretization_manager::ode_sv with scalapack solver");
    break;
...
discretization_manager<precision>::rmse_exact_sol() const
{
  ...
  if (pde->has_analytic_soln())
  {
    ...
    for (auto const &func_batch : exact_funcs)
    {
      ...
      hier.template project_separable<data_mode::replace>
          (solution.data(), pde->get_dimensions(), func_batch, matrices.dv(),
           matrices.mass(), grid, time_, tmult);
    }
