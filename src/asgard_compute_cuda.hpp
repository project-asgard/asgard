...
#define asgard_cuda_check_error(_call_) \
...
#define asgard_cublas_check_error(_call_) \
...
#define asgard_cusolver_check_error(_call_) \
...
inline void set_device(int id) { asgard_cuda_check_error( cudaSetDevice(id) ); }
...
inline T* memalloc(int64_t num) {
  T *data = nullptr;
  asgard_cuda_check_error( cudaMalloc((void**)&data, num * sizeof(T)) );
  return data;
}

template<typename T>
inline void memcopy_dev2dev(int64_t num, T const *src, T *dest) {
  asgard_cuda_check_error( cudaMemcpy(dest, src, num * sizeof(T), cudaMemcpyDeviceToDevice) );
}

template<typename T>
inline void memcopy_host2dev(int64_t num, T const *src, T *dest) {
  asgard_cuda_check_error( cudaMemcpy(dest, src, num * sizeof(T), cudaMemcpyHostToDevice) );
}

template<typename T>
inline void memcopy_dev2host(int64_t num, T const *src, T *dest) {
  asgard_cuda_check_error( cudaMemcpy(dest, src, num * sizeof(T), cudaMemcpyDeviceToHost) );
}
...
void mcopy(int64_t num_entries, device src_dev, T const src[], device dest_dev, T dest[]) {
  asgard_cuda_check_error( cudaMemcpyPeer(dest, dest_dev.id, src, src_dev.id,
...
void copy_to_host(int64_t num_entries, T const x[], std::vector<T> &y) {
  ...
  asgard_cuda_check_error( cudaMemcpy(y.data(), x, num_entries * sizeof(T), cudaMemcpyDeviceToHost) );
}
//! \brief Copy an CPU vector to a device array
template<typename T>
void copy_to_device(std::vector<T> const &x, T y[]) {
  size_t const num_entries = x.size();
  asgard_cuda_check_error( cudaMemcpy(y, x.data(), num_entries * sizeof(T), cudaMemcpyHostToDevice) );
}
...
public:
  ...
  void axpy(int num, no_deduce<P> alpha, P const x[], P y[]) const {
    ...
    if constexpr (is_float<P>) {
      asgard_cublas_check_error( cublasSaxpy(cublas, num, &alpha, x, 1, y, 1) );
    } else {
      asgard_cublas_check_error( cublasDaxpy(cublas, num, &alpha, x, 1, y, 1) );
    }
  }
  template<typename P>
  void axpy(int num, P const x[], P y[]) const {
    static_assert(is_float<P> or is_double<P>,
                  "axpy can be called only with floats and doubles");
    asgard_cublas_check_error( cublasSetPointerMode(cublas, CUBLAS_POINTER_MODE_DEVICE) );
    if constexpr (is_float<P>) {
      asgard_cublas_check_error( cublasSaxpy(cublas, num, fone, x, 1, y, 1) );
    } else {
      asgard_cublas_check_error( cublasDaxpy(cublas, num, done, x, 1, y, 1) );
    }
    asgard_cublas_check_error( cublasSetPointerMode(cublas, CUBLAS_POINTER_MODE_HOST) );
  }
  template<typename P>
  void scal(int num, no_deduce<P> alpha, P x[]) const {
    static_assert(is_float<P> or is_double<P>,
                  "axpy can be called only with floats and doubles");
    if constexpr (is_float<P>) {
      asgard_cublas_check_error( cublasSscal(cublas, num, &alpha, x, 1) );
    } else {
      asgard_cublas_check_error( cublasDscal(cublas, num, &alpha, x, 1) );
    }
  }
  ...
  P dot(int num, P const x[], P const y[]) const {
    ...
    if constexpr (is_float<P>) {
      P res = 0;
      asgard_cublas_check_error( cublasSdot(cublas, num, x, 1, y, 1, &res) );
      return res;
    } else {
      P res = 0;
      asgard_cublas_check_error( cublasDdot(cublas, num, x, 1, y, 1, &res) );
      return res;
    }
  }
  ...
  P nrm2(int num, P const x[]) const {
    ...
    if constexpr (is_float<P>) {
      P res = 0;
      asgard_cublas_check_error( cublasSnrm2(cublas, num, x, 1, &res) );
      return res;
    } else {
      P res = 0;
      asgard_cublas_check_error( cublasDnrm2(cublas, num, x, 1, &res) );
      return res;
    }
  }
  ...
  void gemtv(int m, int n, P alpha, P const A[], P const x[], P beta, P y[]) const {
    if constexpr (is_float<P>) {
      ...
      asgard_cublas_check_error( cublasDgemv(cublas, CUBLAS_OP_T, m, n, &alpha, A, m, x, 1, &beta, y, 1) );
    }
  }
  ...
  void gemv(int m, int n, P alpha, P const A[], P const x[], P beta, P y[]) const {
    if constexpr (is_float<P>) {
      asgard_cublas_check_error( cublasSgemv(cublas, CUBLAS_OP_N, m, n, &alpha, A, m, x, 1, &beta, y, 1) );
    } else {
      asgard_cublas_check_error( cublasDgemv(cublas, CUBLAS_OP_N, m, n, &alpha, A, m, x, 1, &beta, y, 1) );
    }
  }
