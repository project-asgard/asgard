    ...
    for (int j = 0; j < num_rows; j++)
    {
      ...
      for (int t=0; t<num_terms; t++)
      {
        for(int d=0; d<dimensions; d++)
          pA[d] = const_cast<P *>( &vA[t][d * vstride + n * n * (ix[d] * num_1d_blocks + iy[d])] );

        reference_kronmult_one(dimensions, n, pA.data(), x + j * tensor_size, y + i * tensor_size);
      }
    }
  }
}
...
{
  auto result = std::make_unique<kronmult_intputs_welem<P>>(dimensions, n,
                                                            num_terms,
                                                            index_range);
  ...
  if (compute_reference)
    reference_kronmult(dimensions, n, result->num_rows(), num_terms, result->elem.data(), result->get_offsets().data(), index_range, result->input_x.data(), result->reference_y.data());
