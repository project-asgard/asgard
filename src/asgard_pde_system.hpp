 ...
 * The data associated with the system is held in an object of the asgard::pde_system class.
 *
 * \par Components
 ...
 *
 * \par
 * The pde_system is object is the main container for the data associated with PDE system.
 * Time-stepping and solvers operate on the pde_system.
 *
 * Internally, the object will also store all relevant information about the sparse grid discretization (e.g., the hash-tables)
 * and will also hold the current PDE state (i.e., the solution at the current time).
 *
 * \internal
 * \par
 * The constructor takes the user-provided descriptions objects,
 * but will convert those into the internal asgard::dimension and asgard::field objects.
 * The pde_system also contains the table associated with the discretization.
 * \b note : wll also contain the operators.
 * \endinternal
 */
...
public:
   ...
   * also computes the relative offsets of each field win the global vector of degrees of freedom.
   *
