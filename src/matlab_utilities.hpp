...
template <typename P>
std::array<fk::vector<P>, 2> legendre (std::vector<P> x, int const degree) {
  ...
  fk::matrix<P> legendre(num_x, degree);
  fk::matrix<P> legendre_prime(num_x, degree);
  ...
  if(degree >= 3) {
    fk::vector<P> legendre_ml = 
	    legendre.extract_submatrix(0, 0, num_x, 1);
    fk::vector<P> legendre_prime_ml =
	    legendre_prime.extract_submatrix(0, 0, num_x, 1);
    fk::vector<P> legendre_n =
	    legendre.extract_submatrix(0, 1, num_x, 1);
  ...
  for(int i = 0; i < columns_left; ++i) {
    ...
    fk::vector<P> legendre_col = ((2*n+1) * product -
	                        n * legendre_ml);
    legendre_col *= factor;
    ...
    std::transform(x.begin(), x.end(),
	           legendre_prime_ml.begin(),
		   product.begin(), std::multiplies<P>());

    fk::vector<P> legendre_prime_col = ((2*n+1) * 
		                (product + legendre_n) -
	                        n * legendre_prime_ml);
    legendre_prime_col *= factor;
  ...
  for(int i = 0; i < degree; ++i) {
    ...
    fk::matrix<P> legendre_sub = 
	    legendre.extract_submatrix(0, i, num_x, 1);
  ...
  auto iter = x.begin();
  while ((iter = std::find_if(iter, x.end(), 
				  [](P elem) { 
				  return elem < static_cast<P>(1.0) || elem > static_cast<P>(1.0); }) 
			  != x.end())) { 
	  int index = std::distance(x.begin(), iter++); 
	  legendre.update_row(index, std::vector<P>(degree, static_cast<P>(0.0)));
	  legendre_prime.update_row(index, std::vector<P>(degree, static_cast<P>(0.0))); 
  }
  

  // "scaling to use normalization"
  legendre *= std::sqrt(2.0);
  legendre_prime *= std::sqrt(2.0);
