...
int main(int argc, char **argv)
{
  ...
  asgard::fk::vector<int> frow_indx(num_rows * num_rows);
  asgard::fk::vector<int> drow_indx(num_rows * num_rows);
  asgard::fk::vector<int> fcol_indx(num_rows * num_rows);
  asgard::fk::vector<int> dcol_indx(num_rows * num_rows);
  ...
  for(int i=0; i<num_rows; i++)
  {
    ...
    {
      frow_indx[i * num_rows + j] = i * tensor_size;
      drow_indx[i * num_rows + j] = i * tensor_size;
      fcol_indx[i * num_rows + j] = j * tensor_size;
      dcol_indx[i * num_rows + j] = j * tensor_size;
    }
  }
#else
  for(int i=0; i<num_rows; i++)
  {
    frow_indx[i] = i * num_rows;
    drow_indx[i] = i * num_rows;
    for(int j=0; j<num_rows; j++)
    {
      fcol_indx[frow_indx[i] + j]= j;
      dcol_indx[drow_indx[i] + j]= j;
    }
  }
  frow_indx[num_rows] = fcol_indx.size();
  drow_indx[num_rows] = dcol_indx.size();
#endif

  asgard::kronmult_matrix<float> fmat(
      dimensions, n, num_rows, num_rows, num_terms,
      asgard::fk::vector<int, asgard::mem_type::const_view>(frow_indx),
      asgard::fk::vector<int, asgard::mem_type::const_view>(fcol_indx),
  ...
  asgard::kronmult_matrix<double> dmat(
      dimensions, n, num_rows, num_rows, num_terms,
      asgard::fk::vector<int, asgard::mem_type::const_view>(drow_indx),
      asgard::fk::vector<int, asgard::mem_type::const_view>(dcol_indx),
