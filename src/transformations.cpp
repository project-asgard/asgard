...
fk::matrix<R> operator_two_scale(dimension<P> const dim)
{
  ...
  for (int j = 1; j <= n; j++)
  {
    ...
    else
    {
      ...
      cfmwt.set_submatrix(0, 0,
          fk::matrix<R,mem_type::view>(fmwt, 0, cn/2 - 1, 0, cn-1));
      cfmwt.set_submatrix(cn / 2, 0,
          fk::matrix<R,mem_type::view>(fmwt, degree * max_level/2, degree * max_level/2 + cn/2 - 1, 0, cn-1));
    }
